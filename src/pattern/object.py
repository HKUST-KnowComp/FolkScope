...
class Eventuality(JsonSerializedObject):
    ...
    def update(self, x):
        ...
        :type x: Union[float, Eventuality]
    ...
    def phrases_ners(self):
        ...
        for _range in self._phrase_segment_indices:
            if isinstance(self._ners[_range[0]], str):
                ner = self._ners[_range[0]]
            else:
                for x in self._ners[_range[0]].most_common():
                    if x[0] != "O":
                        ner = x[0]
                        break
            _phrases_ners.append(ner)
...
class ASERConcept(JsonSerializedObject):
    ...
    def instantiate(self, kg_conn=None):
        ...
        :type kg_conn: ASERKGConnection
        :return: a list of (eid, pattern, score)
        :rtype: List[Tuple[str, str, float]]
        """

        if kg_conn:
            eventualities = kg_conn.get_exact_match_eventualities([t[0] for t in self.instances])
...
class ASERConceptInstancePair(JsonSerializedObject):
    ...
    def __str__(self):
        repr_dict = {"pid": self.pid, "cid": self.cid, "eid": self.eid, "pattern": self.pattern, "score": self.score}
        return pprint.pformat(repr_dict)
