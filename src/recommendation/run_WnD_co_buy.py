...
class WnD_model(nn.Module):
    ...
    def __init__(self, n_users, n_items, embed_size, features, dropout_rate = 0.5):
...
if __name__ == "__main__":
    ...
    f = open("test_record_WnD_" + domain + "_co_buy_filtered.txt", "w")
    ...
    for run_count in range(5):
        ...
        # "Clothing_Shoes_and_Jewelry_co_buy_transE_feature.json"
        with open(domain + "_co_buy_transE_feature.json", "r") as fin:
        ...
        model_dot = WnD_model(n_users, n_items, 50, TransE_feature_embeddings)
        model_dot.cuda()
        model_dot.cuda()
